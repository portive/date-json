...
import { DateJsonValue, DateJsonToJson, JsonToDateJson } from "./types"
export * from "./types"

/**
 * DateJson Overview
 *
 * The purpose of the DateJson code is to allow us to transport Date data from the
 ...
 * performance.
 */
export function toJsonValue<T extends DateJsonValue>(
  value: T
): DateJsonToJson<T> {
  if (typeof value === "object") {
    ...
    throw new Error(
      `The value ${value} with typeof ${typeof value} is not a valid DateJson value`
    )
  }
}

export function fromJsonValue<T extends JsonValue>(
  value: T
): JsonToDateJson<T> {
  if (typeof value === "object") {
    if (value === null) {
    ...
    throw new Error(`Type ${typeof value} is not a valid DateJson value`)
  }
}
