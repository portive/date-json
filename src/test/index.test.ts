...
describe("ejson", () => {
  describe("JsonDate.toJsonValue", () => {
    ...
    it("should export objects with date", async () => {
      const DateJsonson = { at: new Date("January 1, 2000") }
      const json = toJsonValue(DateJsonson)
    ...
    it("should export arrays with date", async () => {
      const DateJsonson = [new Date("January 1, 2000")]
      const json = toJsonValue(DateJsonson)
    ...
    it("should disallow the key $date in an object", async () => {
      const DateJsonson = { $date: "string-date" }
      expect(() => toJsonValue(DateJsonson)).toThrow(
